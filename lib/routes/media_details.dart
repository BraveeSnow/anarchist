  ...
  return SizedBox(
    ...
    child: BarChart(
      BarChartData(
        ...
        borderData: FlBorderData(show: false), // Removes borders
        barTouchData: BarTouchData(
          ...
          allowTouchBarBackDraw: true, // Ensures background is redrawn on touch
          handleBuiltInTouches: true, // Enables touch interactions for tooltips
        ),
      ),
    ),
  );
...
Widget _buildStatusDistributionGraph(List<StatusDistribution> statusData) {
  ...
  final smallSegmentsTotal = statusData.where((s) => s.amount < 1000).fold(0, (sum, s) => sum + s.amount);

  if (smallSegmentsTotal > 0) {
    largeSegments.add(StatusDistribution(status: "OTHER", amount: smallSegmentsTotal));
  }
